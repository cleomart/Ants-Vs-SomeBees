...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place is not hive or current_place.entrance is not None:
        	if num_turn >= self.min_range and num_turn <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            ...
            [bee.reduce_armor(self.damage) for bee in copy_bees]
...
class WallAnt(Ant):
	"""WallAnt has a large armor value, but does nothing each turn"""
	name = 'Wall'
	food_cost = 4
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        # BEGIN Problem 10
        if self.place.bees is not []:
            copy = self.place.bees[:]
            [bee.reduce_armor(self.damage) for bee in copy]
...
# END Problem 13
    ...
    implemented = True  # Change to True to view in the GUI
    food_cost = 7
    queen = None        # Holds the one and only true queen
    # END Problem 13

    def __init__(self):
        # BEGIN Problem 13
        if QueenAnt.queen == None:
        ...
        if QueenAnt.queen != self:
            ...
            while current_place is not None:
                ...
                if current_ant is not None:
                    if current_ant.container and current_ant.ant != None:
                        if current_ant not in Ant.double_damaged_ants:
                            current_ant.damage *= 2
                            Ant.double_damaged_ants.append(current_ant)
                        if current_ant.ant not in Ant.double_damaged_ants:
                            current_ant.ant.damage *= 2
                            Ant.double_damaged_ants.append(current_ant.ant)
                    else:
                        if current_ant not in Ant.double_damaged_ants:
                            current_ant.damage *= 2
                            Ant.double_damaged_ants.append(current_ant)
